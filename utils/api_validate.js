...
const loadTokenmConfig = () => {
    try {
     ...
     //   console.log(`[validateBasicAuth] 配置文件加载成功，内容如下:`, config);
        return config;
    } catch (error) {
      //  console.error(`[validateBasicAuth] 读取 tokenm.json 文件失败: ${error.message}`);
      //  console.error(`[validateBasicAuth] 配置文件路径: ${tokenmFilePath}`);
        return {}; // 返回空对象以避免程序崩溃
    }
};
...
export const validateBasicAuth = (request, reply, done) => {
 ...
 //   console.log(`[validateBasicAuth] 请求路径: ${request.url}`);
  //  console.log(`[validateBasicAuth] 配置文件中的用户名: ${validUsername}, 密码: ${validPassword}`);

    if (!validUsername && !validPassword) {
        console.log('[validateBasicAuth] 配置文件中未找到用户名和密码，跳过验证');
        done();
        return;
    }
    ...
    if (request.url.startsWith('/config/')) {
        ...
        if (!['index.js', 'index.js.md5', 'index.config.js', 'index.config.js.md5'].includes(cf_path)) {
            console.log(`[validateBasicAuth] 跳过验证: ${cf_path}`);
            done();
            return;
        }
        console.log(`[validateBasicAuth] 验证配置文件: ${cf_path}`);
    }
  ...
  //  console.log(`[validateBasicAuth] 请求用户名: ${username}, 请求密码: ${password}`);

    if (username === validUsername && password === validPassword) {
        console.log('[validateBasicAuth] 验证通过');
        done(); // 验证通过，继续处理请求
    } else {
        reply.header('WWW-Authenticate', 'Basic');
...
export const validatePwd = async (request, reply) => {
    ...
    if (!apiPwd) {
        console.log('[validatePwd] 未配置 API_PWD，直接通过');
    ...
    if (request.url.startsWith('/config/')) {
        const cf_path = request.url.slice(8).split('?')[0];
        if (['index.js', 'index.js.md5', 'index.config.js', 'index.config.js.md5'].includes(cf_path)) {
            console.log(`[validatePwd] 配置文件 ${cf_path} 跳过接口密码鉴权`);
            return;
        }
    }
