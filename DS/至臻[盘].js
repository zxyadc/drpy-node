...
lazy: async function (flag, id, flags) {
        ...
        //    urls.push("原代本1", `http://127.0.0.1:5757/?${threadParam}&form=urlcode&randUa=1&url=` + encodeURIComponent(down.download_url) + '&header=' + encodeURIComponent(JSON.stringify(headers)));
       //    urls.push("原画1", `${down.download_url}`)
        //  urls.push("原画2", `http://127.0.0.1:7777/mediaProxy?${threadParam}&form=urlcode&randUa=10&url=${encodeURIComponent(down.download_url)}`);
        //  urls.push("原画3", `http://127.0.0.1:9975/proxy?do=mutThread&${threadParam}&chunkSize=400&url=${encodeURIComponent(down.download_url)}`);
          urls.push("原画", `http://127.0.0.1:5575/proxy?${threadParam}&chunkSize=256&url=${encodeURIComponent(down.download_url)}`)
           // urls.push("原画4", `http://127.0.0.1:9979/multithread?url=${down.download_url}#fastPlayMode##${threadParam}`)
