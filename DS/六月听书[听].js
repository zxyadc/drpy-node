...
var rule = {
    ...
    play_parse: true,
  //  lazy: $js.toString(() => {
lazy: async function () {  // 将匿名函数转换为具名的异步函数
let {input} = this;
console.log('input的结果:', input);
    ...
    let html = await request('http://www.6yueting.com/web/index/video_new?code=' + data.code + '&no=' + data.no + '&type=' + data.type + '&timestamp=' + data.timestamp + '&sign=' + data.sign);  // 使用 await 等待请求完成
   ...
   // 一级: '.list-wrapper&&.item-link;h2--span&&Text;img&&src;.status&&Text;a&&href',
    ...
    let d = [];
    let data = pdfa(html, '.list-wrapper&&.item-link');
    data.forEach((it) => {
        d.push({
            title: pdfh(it, 'h2--span&&Text'),
            pic_url: pd(it, 'img&&src'),
            desc: pdfh(it, '.status&&Text'),
            url: pd(it, 'a&&href')
        })
    });
    return setResult(d)
    ...
    二级: async function () {
        ...
        let html = await request(input);
        let VOD = {};
        VOD.vod_name = pdfh(html, '.book-title&&Text');
        VOD.type_name = pdfh(html, '.text:eq(0)&&Text');
        VOD.vod_content = pdfh(html, '.book-intro&&Text');
        VOD.vod_remarks = pdfh(html, '.text:eq(0)&&Text') + pdfh(html, '.text:eq(3)&&Text');
        VOD.vod_year = pdfh(html, '.text:eq(4)&&Text');
     //   VOD.vod_area = pdfh(html, '.text:eq(1)&&Text');
        VOD.vod_actor = pdfh(html, '.text:eq(2)&&Text');
        VOD.vod_director = pdfh(html, '.text:eq(1)&&Text');
        

        let playlist = pdfa(html, '.book-list');
        let tabs = pdfa(html, '.operate-bar&&.total-num');
        console.log('tabs的结果:', tabs);
        let playmap = {};
        tabs.map((item, i) => {
            const form = pdfh(item, 'Text')
          //   console.log('form的结果:', form);
  ...
  搜索:  async function () {
    ...
    list.forEach(it => {
        d.push({
            ...
            img: `http://img.6yueting.com:20001/${it.coverUrlLocal}`,
