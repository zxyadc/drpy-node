globalThis.host1 = 'http://txfpan.top/';
globalThis.host2 = '';
globalThis.host3 = '';
globalThis.host4 = '';
...
二级: async function (ids) {
    ...
    for (const item of $('.module-row-title')) {
        ...
        if (/pan.quark.cn/.test(link)) {
            ...
            if (shareData) {
                ...
                if (videos.length > 0) {
                    quarkPlayform.push('Quark-' + shareData.shareId);
                    quarkPlayurls.push(videos.map((v) => {
                        const list = [shareData.shareId, v.stoken, v.fid, v.share_fid_token, v.subtitle? v.subtitle.fid : '', v.subtitle? v.subtitle.share_fid_token : ''];
                        return v.file_name + '$' + list.join('*');
                    }).join('#'))
                } else {
                    quarkPlayform.push('Quark-' + shareData.shareId);
                    quarkPlayurls.push("资源已经失效，请访问其他资源")
                }
            }
        } 
        if (/drive.uc.cn/.test(link)) {
            ...
            if (shareData) {
                ...
                if (videos.length > 0) {
                    ucPlayform.push('UC-' + shareData.shareId);
                    ucPlayurls.push(videos.map((v) => {
                        const list = [shareData.shareId, v.stoken, v.fid, v.share_fid_token, v.subtitle? v.subtitle.fid : '', v.subtitle? v.subtitle.share_fid_token : ''];
                        return v.file_name + '$' + list.join('*');
                    }).join('#'))
                } else {
                    ucPlayform.push('UC-' + shareData.shareId);
                    ucPlayurls.push("资源已经失效，请访问其他资源")
                }
            }
        }
    }
    ...
    // 去除后缀
    let processedQuarkArray = quarkPlayform.map(str => str.replace(/-[\w]+$/, "").replace(/UC/, "优汐").replace(/Quark/, "夸克"));
    let processedUCArray = ucPlayform.map(str => str.replace(/-[\w]+$/, "").replace(/UC/, "优汐"));


    // 处理重复元素
    let uniqueQuarkArray = [];
    let quarkCount = {};
    processedQuarkArray.forEach((item) => {
        if (!quarkCount[item]) {
            quarkCount[item] = 1;
            uniqueQuarkArray.push(item + '#' + quarkCount[item]);
        } else {
            quarkCount[item]++;
            uniqueQuarkArray.push(item + '#' + quarkCount[item]);
        }
    ...
    let uniqueUCArray = [];
    let ucCount = {};
    processedUCArray.forEach((item) => {
        if (!ucCount[item]) {
            ucCount[item] = 1;
            uniqueUCArray.push(item + '#' + ucCount[item]);
        } else {
            ucCount[item]++;
            uniqueUCArray.push(item + '#' + ucCount[item]);
        }
    ...
    // 确保夸克排在前面
    let allPlayform = uniqueQuarkArray.concat(uniqueUCArray);
    allPlayform.sort((a, b) => {
        const aIsKuaKe = a.startsWith("夸克");
        const bIsKuaKe = b.startsWith("夸克");
        if (aIsKuaKe &&!bIsKuaKe) return -1;
        if (!aIsKuaKe && bIsKuaKe) return 1;
        return 0;
    });

    // 先拼接夸克播放链接，再拼接 UC 播放链接
    VOD.vod_play_url = quarkPlayurls.join("$$$") + "$$$" + ucPlayurls.join("$$$");
    
    VOD.vod_play_from = allPlayform.join("$$$");
...
lazy: async function (flag, id, flags) {
        ...
        if (flag.startsWith('夸克')) {
          ...
          //  urls.push("原画", `http://127.0.0.1:7777/mediaProxy?${threadParam}&form=urlcode&randUa=1&url=${encodeURIComponent(down.download_url)}`);
